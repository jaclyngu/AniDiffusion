...
class Pose2ImagePipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # Prepare canonical_pose condition image
        canonical_pose_cond_tensor = self.cond_image_processor.preprocess(
            canonical_pose_image, height=height, width=width
        )
        canonical_pose_cond_tensor = canonical_pose_cond_tensor.unsqueeze(2)  # (bs, c, 1, h, w)
        canonical_pose_cond_tensor = canonical_pose_cond_tensor.to(
            device=device, dtype=self.pose_guider.dtype
        )
        canonical_pose_fea = self.pose_guider(canonical_pose_cond_tensor)
        canonical_pose_fea = (
            torch.cat([canonical_pose_fea] * 2) if do_classifier_free_guidance else pose_fea
        )
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                noise_pred = self.denoising_unet(
                    ...
                    t,
                    encoder_hidden_states=image_prompt_embeds,
                    pose_cond_fea=pose_fea+canonical_pose_fea,
                    return_dict=False,
                )[0]
