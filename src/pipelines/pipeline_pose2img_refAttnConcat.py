...
class Pose2ImagePipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # Jac: Prepare ref image pose latents
        ref_pose_image_tensor = self.ref_image_processor.preprocess(
            ref_pose_image, height=height, width=width
        )  # (bs, c, width, height)
        ref_pose_image_tensor = ref_pose_image_tensor.to(
            dtype=self.vae.dtype, device=self.vae.device
        )
        ref_pose_image_latents = self.vae.encode(ref_pose_image_tensor).latent_dist.mean
        ref_pose_image_latents = ref_pose_image_latents * 0.18215  # (b, 4, h, w)
        net_input=torch.cat((ref_image_latents, ref_pose_image_latents), 1)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if i == 0:
                    for c in range(net_input.size(1)//4):
                        self.reference_unet(
                            net_input[:, c*4:(c+1)*4].repeat(
