...
class Pose2ImagePipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        num_channels_latents = 4#self.denoising_unet.in_channels
        ...
        bg_image_tensor = self.ref_image_processor.preprocess(
            bg_image, height=height, width=width
        )  # (bs, c, width, height)
        bg_image_tensor = bg_image_tensor.to(
            dtype=self.vae.dtype, device=self.vae.device
        )
        bg_image_latents = self.vae.encode(bg_image_tensor).latent_dist.mean
        bg_image_latents = bg_image_latents * 0.18215  # (b, 4, h, w)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latent_model_input=torch.cat((latents, bg_image_latents.unsqueeze(2)), 1)
