...
class UNet3DConditionModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        """
        # print('before', sample.shape, timestep.shape, encoder_hidden_states.shape, self.mode )
        ...
        if self.config.center_input_sample:
            sample = 2 * sample - 1.0
        # print('before 1', sample.shape, timestep.shape, encoder_hidden_states.shape, self.mode )
        # time
        timesteps = timestep
        ...
        # print('before 2', sample.shape, timestep.shape, encoder_hidden_states.shape, self.mode )
        if self.class_embedding is not None:
            ...
            emb = emb + class_emb
        # print('before 3', sample.shape, timestep.shape, encoder_hidden_states.shape, self.mode )
        # pre-process
        sample = self.conv_in(sample)
        if pose_cond_fea is not None:
            sample = sample + pose_cond_fea
            # print(sample.size(),pose_cond_fea.size())

        # print('after', sample.shape, emb.shape, encoder_hidden_states.shape, self.mode )
        # down
    ...
    def from_pretrained_2d(
        ...
        x,y,h,w = model.conv_in.weight.size()
        additional_c =y-4
        state_dict['conv_in.weight']=torch.cat((state_dict['conv_in.weight'], torch.zeros((x,additional_c, h, w))), 1)
