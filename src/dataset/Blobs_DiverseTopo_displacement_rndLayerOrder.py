...
class ChDataset(Dataset):
    ...
    def gen_drawing_info(self):
        ...
        if not different_texture_per_limb:
            ...
            elif texture_source=='Fractal_Noise': #no longer maintained
    ...
    def assign_bfs_depth(self, root):
        ...
        while frontier:
            visit_node=frontier.pop(0)
            #the smaller the order is, the darker the blue is (closer to zero), the more bottom that layer is.
    ...
    def update_graph(self):
        ...
        for i in range(1, limb_num+1):
            anchor=np.random.choice(target_joints)
            x='x'+str(i)
