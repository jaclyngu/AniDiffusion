...
class ChDataset(Dataset):
    def __init__(self, config=None):
        ...
        self.sample_size = np.array([256, 256])
    ...
    def __getitem__(self, idx):
        if len(self.data)==1:
            ...
            if self.config.get('mod', -1) != -1:
                print(idx, self.config['mod'], len(self.data))
                idx=idx % self.config['mod']
            if self.config.get('fix_getitem_index', None):
                idx=(self.config['fix_getitem_index'])[idx]
        print('__getitem__ index from GaussianSkeleton_DiverseAppearance_json_OpenPose', idx)
    ...
    def plot_joint_skeleton_openpose(self, joint_kp, frame):
        size=max(len(joint_kp), len(self.connected_joints))
        subset=-np.ones((size, ))
        limbSeq=[]
        mapped_joint_kp=np.zeros((size, 2))
        # print(self.connected_joints)
