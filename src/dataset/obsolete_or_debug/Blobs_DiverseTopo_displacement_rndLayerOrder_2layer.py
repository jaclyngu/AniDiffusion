...
class ChDataset(Dataset):
    def __init__(self, config=None):
        ...
        self.max_limb_num=int(self.config.get("limb_num", 20))
        self.rndLayerOrder=int(self.config.get("rndLayerOrder", 0))
        self.target_source_same_bg=self.config.get("target_source_same_bg", True)
    ...
    def gen_drawing_info(self):
        #the first one will be composed to form canonical appearance, 
        #the second will be composed to form target
        self.background_imgs=[None, None]
        for i in range(2):
            if self.background_dir[i] is not None:
                if os.path.isfile(self.background_dir[i]):
                    self.background_imgs[i]=Image.open(self.background_dir[i]).resize(self.sample_size, 0).convert('RGB')
                elif os.path.isdir(self.background_dir[i]):
                    Cartoon_Dataset_image_category = np.random.choice(os.listdir(self.background_dir[i]))
                    category_dir=os.path.join(self.background_dir[i], Cartoon_Dataset_image_category)
                    ...
                    self.background_imgs[i]=np.asarray(background_img).astype('float')/255
            else:
                if self.target_bg_color=='black':
                    self.background_imgs[i]=np.zeros((self.sample_size[0], self.sample_size[1], 3))
                elif self.target_bg_color=='white':
                    self.background_imgs[i]=np.ones((self.sample_size[0], self.sample_size[1], 3))

        if self.target_source_same_bg: #keep the first one
            self.background_imgs=[self.background_imgs[0]]*2

        limb_num_max = self.max_limb_num
        ...
        if not different_texture_per_limb:
            ...
            if texture_source=='Cartoon_Dataset':
                ...
                texture_file=np.asarray(Image.open(Cartoon_Dataset_image_path).convert('RGB').resize(self.sample_size))
                ...
                texture_file=np.asarray(Image.open(os.path.join(self.FractalNoise_Dataset_path,Fractal_Noise_image_path)).convert('RGB').resize(self.sample_size))
    ...
    def update_graph(self):
        ...
        if self.skeleton_layer_order:
            ...
            intervals=np.linspace(0, 1, self.max_limb_num+2)[1:]
    ...
    def plot_node_joint_bone(self, node, limb_num, intervals):
        ...
        if intervals is not None:
            ...
            depth_blue= intervals[node.layer_order] #intervals[limb_num-node.layer_order-1] #+np.random.uniform(-0.3*equi_interval, 0.3*equi_interval)
    ...
    def __getitem__(self, idx):
        if self.count% self.change_graph_freq==0:
            # print('change graph!', idx, self.count)
            self.update_graph()
        else:
            # print('change no!', idx, self.count)
            for i in self.nodes:
                i.move=False
        ...
        background_img_np=self.background_imgs[1]
        background_img=self.transform(Image.fromarray((background_img_np*255).astype('uint8')))
        ...
        if self.rndLayerOrder>0:
            if self.n_targets<=0: #means it's not initialized yet
                more_target_list, more_vis_list=[],[]
            more_canonical_target_list, more_canonical_vis_list=[canonical_target],[canonical_vis]
            for t in range(self.rndLayerOrder):
                rndLayerOrder=np.arange(len(self.nodes))
                if t==0:
                    rndLayerOrder=rndLayerOrder[::-1].tolist()
                else:
                    np.random.shuffle(rndLayerOrder)
                    rndLayerOrder=rndLayerOrder.tolist()
                
                more_canonical_data=self.getonepair(-1, rndLayerOrder=rndLayerOrder)
                more_canonical_target_list.append(more_canonical_data["pixel_values"])
                more_canonical_vis_list.append(more_canonical_data['visualize_target'])

                ref_idx=range(self.n_pairs+1, self.n_targets+self.n_pairs+1)
                for i in [0]+list(ref_idx):
                    more_target_data=self.getonepair(i, rndLayerOrder=rndLayerOrder)
                    more_target_list.append(more_target_data["pixel_values"])
                    more_vis_list.append(more_target_data['visualize_target'])
        if self.rndLayerOrder>0 or self.n_targets>0:
            target=torch.stack([target]+more_target_list)
            target_vis=torch.stack([target_vis]+more_vis_list)
            if self.rndLayerOrder<=0:
                canonical_vis=torch.stack([canonical_vis]*((1+self.n_targets)*(self.rndLayerOrder+1)))
                canonical_target=torch.stack([canonical_target]*((1+self.n_targets)*(self.rndLayerOrder+1)))
            else:
                canonical_vis=torch.stack([i for i in more_canonical_vis_list for j in range(1+self.n_targets)])
                canonical_target=torch.stack([i for i in more_canonical_target_list for j in range(1+self.n_targets)])
            ref_vis=torch.stack([ref_vis]*((1+self.n_targets)*(self.rndLayerOrder+1)))
            ref_target=torch.stack([ref_target]*((1+self.n_targets)*(self.rndLayerOrder+1)))
            clip_image=torch.stack([clip_image]*((1+self.n_targets)*(self.rndLayerOrder+1)))
            background_img=torch.stack([background_img]*((1+self.n_targets)*(self.rndLayerOrder+1)))
    ...
    def transform_one_limb(self, node, transform_idx):
        ...
        # input to cv2 wivvdth first height second
    ...
    def getonepair(self, idx, rndLayerOrder=None):
        # assume that even by add, only add limbs with the same skeleton
        if idx==-1:
            rgba=np.copy(self.background_imgs[0])
        else:
            rgba=np.copy(self.background_imgs[1])

        transformed_ordered_nodes, transformed_images, transformed_joint_bones =self.bfs(self.nodes[0], idx)
        if rndLayerOrder:
            # print(len(transformed_ordered_nodes), len(transformed_images), len(transformed_joint_bones), len(rndLayerOrder))
            transformed_ordered_nodes=[transformed_ordered_nodes[i] for i in rndLayerOrder]
            #transformed_images has one less element since it doesn't have for the root node
            transformed_images=[transformed_images[i] for i in rndLayerOrder if i<len(transformed_images)]
            transformed_joint_bones=[transformed_joint_bones[i] for i in rndLayerOrder]
            intervals=np.array([0.33333333, 0.66666667, 1.])#np.linspace(0, 1, self.max_limb_num+2)[1:]
            for b in range(len(transformed_joint_bones)):
                depth_blue=intervals[b] #+np.random.uniform(-0.3*equi_interval, 0.3*equi_interval)
                transformed_joint_bones[b][:,:,2]=np.ones_like(transformed_joint_bones[b][:,:,2])*depth_blue
        ...
        if (idx!=-1) and (rotate_translate_coin<self.config.get("rotate_translate_prob", 0.)):
            random_trans_y=np.random.randint(-self.sample_size[0]//8, high=self.sample_size[0]//8)
            random_trans_x=np.random.randint(-self.sample_size[1]//8, high=self.sample_size[1]//8)
            random_rotate_deg=np.random.uniform(-180, high=180)
            if self.skeleton_bg_color=='black':
