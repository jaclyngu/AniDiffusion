...
# import sys
# sys.path.insert(0, '/sensei-fs/users/diliu/zeqi_project/pytorch-openpose')
...
def draw_bodypose(canvas, candidate, subset):
    ...
    for i in range(len(limbSeq)):
        index = subset[np.array(limbSeq[i]) - 1] #the output is a list, with index=limbSeq-1
        if -1 in index:
            continue
...
class ChDataset(Dataset):
    ...
    def getonepair(self, idx, drawing_info, use_canonical_image=False, no_transform=False):
        ...
        if self.bone_interpolation == 'interpolation':
            visualize_target=self.plot_joint_skeleton_features(skeleton_canvas, visualize_joint_kp, draw_target_joints)
        elif self.bone_interpolation == 'matplotlib':
            visualize_target=self.plot_joint_skeleton(skeleton_canvas, visualize_joint_kp, target_joints, additional_joints)
        elif self.bone_interpolation == 'openpose':
    ...
    def plot_joint_skeleton_openpose(self, joint_kp, frame):
        subset=np.array([ 0.        ,  1.        ,  2.        ,  3.        ,  4.        ,
            ...
            10.        , 11.        , 12.        , 13.        , 14.        ,
            15.        , -1        , -1])
        candidate=np.array([[126.        ,  20.        ,   0.95016873,   0.        ],
            [125.        ,  48.        ,   0.96147305,   1.        ],
            [100.        ,  48.        ,   0.91413134,   2.        ],
            [ 93.        ,  80.        ,   0.9301343 ,   3.        ],
            [ 86.        , 109.        ,   0.910173  ,   4.        ],
            [150.        ,  48.        ,   0.97247624,   5.        ],
            [157.        ,  82.        ,   0.9557144 ,   6.        ],
            [172.        , 112.        ,   0.90500724,   7.        ],
            [110.        , 115.        ,   0.81766987,   8.        ],
            [109.        , 165.        ,   0.89098072,   9.        ],
            [108.        , 217.        ,   0.91228741,  10.        ],
            [142.        , 116.        ,   0.8361541 ,  11.        ],
            [141.        , 165.        ,   0.89802319,  12.        ],
            [140.        , 217.        ,   0.92106938,  13.        ],
            [121.        ,  17.        ,   0.95001274,  14.        ],
            [130.        ,  17.        ,   0.9770261 ,  15.        ],
            [115.        ,  21.        ,   0.88638628,  16.        ],
            [137.        ,  19.        ,   0.83064294,  17.        ]])
        mapped_joint_kp=candidate.copy()
        for k, openpose_index in self.openpose_index_map.items():
            if k in joint_kp:
                v=joint_kp[k]
                mapped_joint_kp[openpose_index][0]=v[0]
                mapped_joint_kp[openpose_index][1]=v[1]
            else:
                subset[int(openpose_index)]=-1
        mapped_joint_kp[1][:2]=(mapped_joint_kp[2][:2]+mapped_joint_kp[5][:2])/2
        frame = draw_bodypose(frame.astype('uint8'), mapped_joint_kp, subset)
