...
class ChDataset(Dataset):
    ...
    def gen_drawing_info(self, idx):
        ...
        background_imgs=[None, None]
        for i in range(2):
            if self.background_dir[i] is not None:
                if os.path.isfile(self.background_dir[i]):
                    background_imgs[i]=Image.open(self.background_dir[i]).resize(self.sample_size, 0).convert('RGB')
                    ...
                    background_imgs[i]=np.asarray(background_img).astype('float')/255
            else:
                if self.target_bg_color=='black':
                    background_imgs[i]=np.zeros((self.sample_size[0], self.sample_size[1], 3))
                elif self.target_bg_color=='white':
                    background_imgs[i]=np.ones((self.sample_size[0], self.sample_size[1], 3))

        if self.target_source_same_bg: #keep the first one
            background_imgs=[background_imgs[0]]*2
        ...
        return everywhere_cartoon, background_imgs, flip, resize_ratio, rotate_translate_coin, random_trans_y, random_trans_x, random_rotate_deg, \
                composite_order
    ...
    def __getitem__(self, idx):
        ...
        print('__getitem__ index from GaussianSkeleton_DiverseAppearance_rndLayerOrder', idx, self.data[idx]['image_path'])
        if self.bone_interpolation=='displacement':
        ...
        background_img_np=drawing_info[1][1]
        ...
        if self.sample_n_frames > 0:
            ...
            clip_image=torch.stack([clip_image]*self.sample_n_frames)
    ...
    def getonepair(self, idx, drawing_info, use_canonical_image=False, no_transform=False):
        ...
        everywhere_cartoon, background_imgs, flip, resize_ratio, rotate_translate_coin, random_trans_y, random_trans_x, random_rotate_deg, composite_info = drawing_info
        ...
        if use_canonical_image:
            ...
            background_img=background_imgs[1]
